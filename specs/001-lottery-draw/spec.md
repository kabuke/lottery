...
- **識別機制:** 使用「租戶名稱」+「臨時密碼」作為唯一識別 (取代 IP 綁定)。
...
- **登入機制:**
    - 首頁必須提供一個登入表單，包含兩個欄位：
        - **租戶名稱 (Tenant Name):** 使用者名稱或公司名稱。
        - **臨時密碼 (Temporary Password):** 用於確保資料安全與跨 IP 還原資料。
    - **密碼限制:** 長度必須介於 6 至 18 個字元之間。
- **識別邏輯:**
    - 系統不再使用 Client IP 作為識別依據。
    - 系統將結合 `租戶名稱` 與 `臨時密碼` 產生唯一的 Session ID (Tenant ID)。
    - 當使用者更換設備或 IP 時，只要輸入相同的名稱與密碼，即可存取原本的資料 (Session Recovery)。
...
- **核心邏輯:**
    - 系統必須確保每位參與者最多只能中一個「常規獎項」。
    - 「額外獎項 (DrawFromAll=true)」的中獎不應影響參與「常規獎項」的資格，反之亦然。即：
        - 中了「常規獎項」的人，透過 DrawFromAll 選項仍可抽「額外獎項」。
        - 中了「額外獎項」的人，仍視為尚未中過「常規獎項」，有資格參與常規獎項的抽獎。
- **抽獎介面:**
