...
## 2. 身分識別與 Session (對應 Spec 2.0)

- [x] **任務 2.0.1 (前端):** 更新首頁登入表單，新增「臨時密碼」欄位。
- [x] **任務 2.0.2 (後端):** 實作密碼長度驗證 (6-18 字元)。
- [x] **任務 2.0.3 (後端):** 修改 `POST /set-tenant`，改用 `Name + Password` 生成 Tenant ID。
- [x] **任務 2.0.4 (後端):** 修改 `TenantMiddleware`，移除 IP 依賴，改讀取 Cookie 中的 ID。
...
- [x] **任務 4.6 (資格優化):** 修正抽獎資格判定邏輯，確保「特殊獎項」與「常規獎項」的中獎資格互不排斥。
